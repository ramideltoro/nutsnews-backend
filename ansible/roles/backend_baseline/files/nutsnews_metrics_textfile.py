...
WORKER_UPLIFT_DEPLOYMENT_MODES = {"shadow", "production"}
...
def worker_uplift_ownership_metric_lines() -> list[str]:
    """Export the protected ownership gate independently of worker scrapes."""
    raw_expected_active = os.environ.get("NUTSNEWS_WORKER_UPLIFT_EXPECTED_ACTIVE", "0").strip()
    raw_mode = os.environ.get("NUTSNEWS_WORKER_UPLIFT_DEPLOYMENT_MODE", "shadow").strip().lower()
    environment = os.environ.get("NUTSNEWS_DEPLOYMENT_ENVIRONMENT", "production").strip().lower()
    host = os.environ.get("NUTSNEWS_TELEMETRY_HOST", "backend.nutsnews.com").strip().lower()
    identity_valid = bool(
        re.fullmatch(r"[a-z][a-z0-9_-]{0,31}", environment)
        and re.fullmatch(r"[a-z0-9][a-z0-9.-]{0,252}", host)
    )
    expected_valid = raw_expected_active in {"0", "1"}
    mode_valid = raw_mode in WORKER_UPLIFT_DEPLOYMENT_MODES
    pairing_valid = (
        (raw_mode == "production" and raw_expected_active == "1")
        or (raw_mode == "shadow" and raw_expected_active == "0")
    )
    ...
    return [
        ...
        "# HELP nutsnews_backend_worker_uplift_expected_active Whether worker uplift owns production ingestion.",
        ...
        "# HELP nutsnews_backend_worker_uplift_deployment_info Protected worker-uplift deployment mode.",
...
def running_worker_uplift_label_sets() -> list[dict[str, str]] | None:
    """Read only the bounded immutable identity labels from running containers."""
    try:
    ...
    if containers.returncode != 0:
        return None
    container_ids = [value.strip() for value in containers.stdout.splitlines() if value.strip()]
    if len(container_ids) > 24 or any(not re.fullmatch(r"[0-9a-f]{12,64}", value) for value in container_ids):
        return None
...
def worker_uplift_deployed_identity_metric_lines(
    ...
    """Export identities only when the running containers match the reviewed manifest."""
    ...
    if valid and isinstance(services, list):
        for service in services:
            ...
            revision = str(service.get("build_revision") or "")
            ...
            identity_valid = bool(
                ...
                and service.get("image_tag") == revision
    ...
    observed = running_label_sets if running_label_sets is not None else running_worker_uplift_label_sets()
...
def relay_failed_table_count(data: dict[str, Any]) -> int | None:
    ...
    if isinstance(tables, list):
        return sum(
            1
            for table in tables
            if isinstance(table, dict) and str(table.get("status", "unknown")) not in {"applied", "pass"}
        )
    return None
...
def collect() -> list[str]:
    ...
    relay_status = raw_relay_status if raw_relay_status in {"pass", "fail", "not_configured", "unknown"} else "unknown"
    failed_table_count = relay_failed_table_count(relay)
    last_applied_at = str(relay.get("last_applied_at_utc") or "")
    relay_checked_at = str(relay.get("checked_at_utc") or "")
    relay_completed_at = str(relay.get("completed_at_utc") or "")
    relay_lag_seconds = age_seconds(last_applied_at, now)
    relay_collector_age = age_seconds(relay_checked_at, now)
    relay_collector_fresh = relay_collector_age is not None and relay_collector_age <= 600
    last_success_timestamp = None
    if relay_status == "pass":
        last_success_timestamp = timestamp_seconds(relay_completed_at or relay_checked_at)
    relay_available = relay_status in {"pass", "fail"}
    relay_healthy = (
        relay_status == "pass"
    ...
    lines.extend(
        [
            ...
            "# HELP nutsnews_backend_sync_relay_lag_seconds Real age of the last applied relay change, or -1 when unavailable.",
