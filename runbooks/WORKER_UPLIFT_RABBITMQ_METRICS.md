...
```text
environment, deployment_environment, host, instance, job, service,
service_namespace, rabbitmq_endpoint, node, cluster, vhost, queue, exchange
```
...
rate(rabbitmq_detailed_queue_exchange_messages_published_total{queue=~"nutsnews\\.worker\\..+"}[5m])
```
