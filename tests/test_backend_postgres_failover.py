...
class BackendPostgresFailoverTests(unittest.TestCase):
    ...
    def test_protected_apply_wires_postgres_secrets_without_values(self):
        ...
        self.assertIn('if cutover_state != "shadow":', build_step)
        self.assertIn('if uplift_production_writes != "false":', build_step)
        self.assertIn('"backend_worker_api_uplift_cutover_state"] = "shadow"', build_step)
        self.assertIn('"backend_worker_api_uplift_production_writes_enabled"] = False', build_step)
